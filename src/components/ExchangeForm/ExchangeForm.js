...
class ExchangeForm extends Component {
  ...
  executeExchange(e) {
    ...
    exchangeInvestment({
      username,
      source_investment: parseInt(source_investment),
      target_investment: parseInt(target_investment),
  ...
  updateExchangeRate() {
    ...
    if (source_currency == target_currency)
      ...
      exchange_rate = this.props.exchange_rates[
      ...
      if (!exchange_rate) {
        exchange_rate = this.props.exchange_rates[
          target_currency + "_" + source_currency
        ];
        console.log("t: ", target_currency + "_" + source_currency);
        console.log(
          "t: exchange rate ",
          this.props.exchange_rates[target_currency + "_" + source_currency]
        );

        if (exchange_rate) {
          let new_exchange_rate = {
            bid: 1 / exchange_rate.ask,
            ask: 1 / exchange_rate.bid,
            mid: 1 / exchange_rate.mid
          };
    ...
    this.setState({ exchange_rate }, () => {
      ...
      if (amount != "") {
        this.setState({
          target_amount: parseFloat((amount * exchange_rate.bid).toFixed(8))
        });
      }
    });
  }
    ...
    if (e.target.name == "amount") {
      ...
      this.setState({
        target_amount: parseFloat(
          (e.target.value * exchange_rate.bid).toFixed(8)
        )
      });
      ...
      this.setState({
        amount: parseFloat(
          ((e.target.value * 1) / exchange_rate.ask).toFixed(8)
        )
      });
    } else {
      ...
      if (e.target.name == "source_investment") {
        ...
        let currency = this.findCurrency(
          this.props.user_investments,
          investment_id
        );
        console.log("currency", currency);
        this.setState(
          { source_currency: currency, source_investment: investment_id },
          this.updateExchangeRate
        );
      } else {
        ...
        console.log("currency", currency);
        this.setState(
          { target_currency: currency, target_investment: investment_id },
          this.updateExchangeRate
        );
      }
    }
    ...
    const investmentsOptions = investments.map((investment, idx) => {
      ...
      if (idx == 0) {
        if (
          ...
          this.state.source_investment == ""
        )
          this.setState({
            source_currency: investment.currency,
            source_investment: investment.investment_id
          });
      }
      ...
      if (idx == 1) {
        if (
          ...
          this.state.target_investment == ""
        )
          this.setState({
            target_currency: investment.currency,
            target_investment: investment.investment_id
          });
      }
      ...
      if (investment.investment_id == hidden_investment_id) {
        return (
          ...
          >
            {investment.investment_name + " (" + investment.currency + ")"}
          </option>
        );
      } else {
        return (
  ...
  render() {
    ...
    const userInvestmentList = this.generateInvestmentList(
      this.props.user_investments,
      target_investment,
      "source"
    );
    const allInvestmentList = this.generateInvestmentList(
          ...
          <div className="form justify-content-center">
            <form onSubmit={this.executeExchange}>
              <Row className="justify-content-center">
                <Col xs={1} md={1} lg={1} className="form-group no-padding">
                  Buy
                </Col>
                <Col xs={5} md={2} lg={3} className="form-group no-padding">
                  <select
                    className="form-control Trans-form-control"
                    name="source_investment"
                    required
                    value={source_investment}
                ...
                <Col xs={5} md={2} lg={1} className="form-group no-padding">
                  <input
                    type="text"
                    className="form-control Trans-form-control"
                    id="amount"
                ...
                <Col xs={12} md={2} lg={2} className="form-group">
                  <button
                    ...
                    type="submit"
                    name="exchange"
                    className="btn btn-info transfer-btn"
                  >
                    Exchange
                  </button>
                </Col>
